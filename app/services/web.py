...
from exceptions.chats import ChatListRequestError, ChatListenerListRequestError, ChatListenerAddRequestError, \
    ChatInfoRequestError
from services.constants import CHAT_LIST_URI, CHAT_LISTENERS_URI, DEFAULT_LIMIT, DEFAULT_OFFSET, CHAT_INFO_URI
...
class ChatWebService(BaseChatWebService):
    ...
    async def get_chat_listeners(self, chat_oid: str) -> list[ChatListenerDTO]:
        ...
        if not response.is_success:
            raise ChatListenerListRequestError(
    ...
    async def add_listener(self, telegram_chat_id: int, chat_oid: str):
        ...
        if not response.is_success:
            raise ChatListenerAddRequestError(
                status_code=response.status_code,
                response_content=response.content.decode()
            )
